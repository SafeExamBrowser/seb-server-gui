...
export async function addTableRow(token: string, templateId: string, params: {}): Promise<[object, number]> {
...
export async function deleteTableRow(token: string, templateId: string, params: {}): Promise<[object, number]> {
    const url: string =  constants.TEMPLATE_SEB_SETTINGS_ENDPOINT + "/" + templateId + "/table/row";
    const {data, status} = await apiService.api.delete(url, {headers: apiService.getHeaders(token), params});
    return [data, status];
}

export async function updateSEBSettingsValue(token: string, templateId: string, valueId: string, value: string): Promise<[object, number]> {
    const url: string =  constants.TEMPLATE_SEB_SETTINGS_ENDPOINT + "/" + templateId;
