<template>
    <v-snackbar 
        ...
        :timeout="errorStore.errorProps.timeout ? errorStore.errorProps.timeout : 5000">
        ...
        <template v-slot:actions>
            <v-btn v-if="errorStore.errorProps.details" variant="text" @click="isDetails = !isDetails"> 
                <template v-if="!isDetails">
                    Show Details
                </template> 
                <template v-else>
            ...
            <v-btn variant="text" @click="errorStore.isError = false"> Close </v-btn>
        </template>
    </v-snackbar>
</template>
...
<script setup lang="ts">
    ...
    function getText(): string{
        if(errorStore.errorProps.textKey){
            messages.find(item => item.key == errorStore.errorProps.textKey)?.value;
        }
