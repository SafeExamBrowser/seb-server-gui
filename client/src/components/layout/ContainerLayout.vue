<template>
        ...
        <v-menu :close-on-content-click="false">
          ...
          <v-list>
            <v-list-item class="d-flex">
              ...
              <v-list-item-title>{{ $t('navigation.loggedInAs') }} {{ userAccountStore.userAccount?.institutionId }}</v-list-item-title>
  ...
  <v-navigation-drawer v-model="navigationDrawer" :permanent="true" width="70">
    <v-list lines="two">
      <v-list-item
        v-if="canAccessNavigationOverview"
        link
        elevation="0"
...
<script setup lang="ts">
  ...
  const ALLOWED_ROLES = ["INSTITUTIONAL_ADMIN", "SEB_SERVER_ADMIN"];
