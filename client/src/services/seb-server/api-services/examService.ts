...
export async function createExam(createExamPar: CreateExamPar): Promise<Exam | any>{
    return (await apiService.api.post(examUrl, createExamPar, {headers: apiService.getPostHeaders("accessToken")})).data;
}

export async function deleteExam(id: string): Promise<any | null>{
    return (await apiService.api.delete(examUrl + "/" + id, {headers: apiService.getHeaders("accessToken")})).data;
}

export async function updateExam(id: string, exam: Exam): Promise<Exam | any>{
    return (await apiService.api.put(examUrl + "/" + id, exam, {headers: apiService.getPostHeaders("accessToken")})).data;
}

export async function getExams(optionalParamters?: OptionalParGetExams): Promise<Exams | any>{
    return (await apiService.api.get(examsUrl, {headers: apiService.getHeaders("accessToken"), params: {optionalParamters}})).data;
}

export async function archiveExam(id: string): Promise<Exam | any>{
    return (await apiService.api.patch(examUrl + "/" + id + "/archive", {headers: apiService.getHeaders("accessToken")})).data;
}
...
export async function applySEBLock(id: string, enableSEBLock: boolean): Promise<Exam | null>{
    ...
    if (enableSEBLock) {
        return (await apiService.api.put(url, {}, {headers: apiService.getHeaders("accessToken")})).data;
    } else {
        return (await apiService.api.delete(url, {headers: apiService.getHeaders("accessToken")})).data;
    }
}